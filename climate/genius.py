...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for zone in zone_list:
        ...
        async_add_entities([GeniusDevice(genius_utility, name, id, mode,
                                         target_temperature, current_temperature)])
    _LOGGER.info("Genius Climate setup Complete!")
...
class GeniusDevice(ClimateDevice):
    ...
    def set_operation_mode(self, operation_mode):
        ...
        self._genius_utility.putjson('/zones/' + str(self._device_id) + '/mode', self._current_operation)
    ...
    def set_temperature(self, **kwargs):
        ...
        if kwargs.get(ATTR_TEMPERATURE) is not None:
            ...
            self._genius_utility.putjson('/zones/' + str(self._device_id) + '/override', {'duration': 900, 'setpoint': self._target_temperature})

    async def async_update(self):
        """Get the latest data."""
        _LOGGER.info("GeniusClimate update called!")
        data = self._genius_utility.getZone(self._device_id)
        if data:
    ...
    async def async_turn_on(self, **kwargs):
        ...
        self._genius_utility.putjson('/zones/' + str(self._device_id) + '/mode', "override")

    async def async_turn_off(self, **kwargs):
        """Turn off."""
        _LOGGER.info("GeniusClimate turn off called!")
        self._genius_utility.putjson('/zones/' + str(self._device_id) + '/mode', "off")
